...
object Churn {
  ...
  def main(args: Array[String]) {
    ...
    val train: Dataset[Account] = spark.read.option("inferSchema", "false").option("header","true").schema(schema).csv("hdfs://"+args(0)+":9000/churn-bigml-80.csv").as[Account]
    //val train: Dataset[Account] = spark.read.option("inferSchema", "true").csv("hdfs://"+args(0)+":9000/churn-bigml-80.csv").as[Account]
    train.take(1)
    train.cache
    println(train.count)

    val test: Dataset[Account] = spark.read.option("inferSchema", "false").option("header","true").schema(schema).csv("hdfs://"+args(0)+":9000/churn-bigml-20.csv").as[Account]
    test.take(2)
    println(test.count)
    test.cache
